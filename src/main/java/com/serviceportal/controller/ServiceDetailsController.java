...
public class ServiceDetailsController {
    ...
    @RequestMapping(value = "ServiceDetails/{id}", method = RequestMethod.POST)
    public ServiceDetails save(@PathVariable Long id, @RequestBody ServiceDetails serviceDetails) {
        ServiceDetails existingServiceDetails = ServiceDetailsRepository.findOne(id);
        System.out.println("I am in this post");
    ...
    @RequestMapping(value = "ServiceDetails/{id}/{status}", method = RequestMethod.POST)
    public ServiceDetails update(@PathVariable Long id, @PathVariable Boolean status) {
        ServiceDetails existingServiceDetails = ServiceDetailsRepository.findOne(id);
        //BeanUtils.copyProperties(ServiceDetails, existingServiceDetails);
        System.out.println("Found one in the database");
        if (status != null) {
            existingServiceDetails.setFlag(status.toString());
        }
        return ServiceDetailsRepository.saveAndFlush(existingServiceDetails);
    }
